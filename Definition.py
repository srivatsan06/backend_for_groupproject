...
class TableDefinition:
    ...
    def table_definition(self):
        try:
            ...
            self.cursor.execute("CREATE TABLE student (`stud_id` varchar(8) PRIMARY KEY, `stud_name` varchar(30) NOT NULL, `year` INT NOT NULL, `email` varchar(40), `course_id` varchar(6) , FOREIGN KEY (course_id) REFERENCES course(course_id) ON UPDATE CASCADE ON DELETE CASCADE );")
            self.cursor.execute("CREATE table users(`user_id` varchar(8) PRIMARY KEY, `user_name` varchar(30) NOT NULL, `role` ENUM ('module_staff','welfare_staff') NOT NULL, `email` varchar(40) );")
